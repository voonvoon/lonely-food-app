...
export const createPaymentLinkPost = async (items: any): Promise<{ url: string; data: any; }> => {
  const totalAmount = items
    ...
    // const generateRandomOrderId = (): string => {
    //   const now = new Date();
    //   const dateString = now.toISOString().replace(/[-:.TZ]/g, ""); // Remove unwanted characters
    //   const uniqueSuffix = Math.floor(Math.random() * 10000).toString().padStart(4, '0'); // Ensure uniqueness
